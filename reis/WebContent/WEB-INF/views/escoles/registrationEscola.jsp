...
<html lang="ca">
	<head>
		...
		<c:if test="${editMode == true}">
			<title>Reis d'Igualada :: Editar escola</title>
		</c:if>
		<c:if test="${editMode == false}">
			<title>Reis d'Igualada :: Crear escola</title>
		</c:if>
	...
	<body>
		...
		<c:if test="${pageContext.request.userPrincipal.name != null}">
			...
			<sec:authorize access="hasRole('ROLE_ADMIN') || hasRole('ROLE_NENS')">
				<div class="container">
					<form:form method="POST" modelAttribute="escolaForm" class="form-signin">
						...
						<c:if test="${editMode == true}">
							<h2 class="form-signin-heading">Editar escola</h2>
						</c:if>
						<c:if test="${editMode == false}">
							<h2 class="form-signin-heading">Crear escola</h2>
						</c:if>
						<spring:bind path="descripcio">
							<div class="form-group ${status.error ? 'has-error' : ''}">
								<form:input type="text" path="descripcio" class="form-control" placeholder="Escola" autofocus="true"></form:input>
								<form:errors path="descripcio"></form:errors>
								<!-- cssClass="error" -->
							</div>
						</spring:bind>
						...
						<div align="center">
							...
							<c:if test="${editMode == true}">
								...
								<a href="#" class="btn btn-lg btn-danger" title="Eliminar" onclick="eliminarEscola(${escolaForm.id})">Eliminar</a>
							</c:if>
							<c:if test="${editMode == false}">
		...
		<script src="${contextPath}/resources/js/escoles/registre.js"></script>
	</body>
</html>
