...
<html lang="ca">
	<head>
		...
		<c:if test="${editMode == true}">
			<title>Arxiu Reis d'Igualada :: Editar autor</title>
		</c:if>
		<c:if test="${editMode == false}">
			<title>Arxiu Reis d'Igualada :: Crear autor</title>
		</c:if>
	...
	<body>
		...
		<c:if test="${pageContext.request.userPrincipal.name != null}">
			...
			<sec:authorize access="hasRole('ROLE_ADMIN') || hasRole('ROLE_ARXIU')">
				<div class="container">
					<form:form method="POST" modelAttribute="autorForm" class="form-signin">
						...
						<c:if test="${editMode == true}">
							<h2 class="form-signin-heading">Editar autor</h2>
						</c:if>
						<c:if test="${editMode == false}">
							<h2 class="form-signin-heading">Crear autor</h2>
						</c:if>
						<spring:bind path="name">
							<div class="form-group ${status.error ? 'has-error' : ''}">
								<form:input type="text" path="name" class="form-control" placeholder="Autor" autofocus="true"></form:input>
						...
						<div align="center">
							...
							<c:if test="${editMode == true}">
								...
								<a href="#" class="btn btn-lg btn-danger" title="Eliminar" onclick="eliminarAutor(${autorForm.id})">Eliminar</a>
							</c:if>
							<c:if test="${editMode == false}">
		...
		<script src="${contextPath}/resources/js/autors/registre.js"></script>
	</body>
</html>
